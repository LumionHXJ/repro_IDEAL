...
def process_test_examples(examples,label_map_temp):
    processed_examples = []
    idx = 0
    for raw_data in tqdm(examples, desc='process test examples'):
        processed_examples.append({
            'id': idx,
            'label': label_map_temp[raw_data['output']],
...
def process_imdb_examples(examples):
    processed_examples = []
    idx = 0
    for raw_data in tqdm(examples,desc='process imdb examples'):
        processed_examples.append({
            'id': idx,
            'label': raw_data['label'],
            'text': raw_data['text'],
...
def get_task(args):
    ...
    if task_name=='mnli':
        if os.path.isfile(os.path.join(args.output_dir,f'train_examples_seed_{args.seed}.json')) and \
            ...
            total_train_examples = random.sample(total_train_examples, 300) #!
        ...
        if os.path.isfile(os.path.join(args.output_dir, f'train_examples_seed_{args.seed}.json')) and \
            ...
            boolq_datasets = load_dataset('/data/huxingjian/In_Context_Learning/boolq')
            boolq_cst_datasets = load_dataset("json", 
                                              data_files={"test":'/data/huxingjian/In_Context_Learning/boolq/contrast_boolq.jsonl'})
            ...
            total_eval_examples = [e for e in boolq_cst_datasets['test']]
            total_eval_examples = random.sample(total_eval_examples, 256)
        ...
        all_train_text_to_encode = ["{}.\nquestion: {}".format(raw_item["passage"], raw_item["question"])
                                    for raw_item in total_train_examples]
        all_eval_text_to_encode = ["{}.\nquestion: {}".format(raw_item["passage"], raw_item["question"])
                                    for raw_item in total_eval_examples]
        label_map = {0:"True",1:"False"}
        ...
        if os.path.isfile(os.path.join(args.output_dir, f'train_examples_seed_{args.seed}.json')) and \
            ...
            sst5_datasets = load_dataset(data_cache_dir,cache_dir=data_cache_dir)
        ...
        if os.path.isfile(os.path.join(args.output_dir, f'train_examples_seed_{args.seed}.json')) and \
            ...
            sst2_datasets = load_dataset('glue', 'sst2', cache_dir="/data/huxingjian/In_Context_Learning/sst2")
            imdb_datasets = load_dataset("/data/huxingjian/In_Context_Learning/imdb")
            total_train_examples = [e for e in sst2_datasets['train']]
            total_train_examples = random.sample(total_train_examples, 3000)
            total_train_examples = process_sst2_examples(total_train_examples)
            total_eval_examples = [e for e in imdb_datasets['test']]
            total_eval_examples = random.sample(total_eval_examples, 256)
            total_eval_examples = process_imdb_examples(total_eval_examples)
            with open(os.path.join(args.output_dir, f'train_examples_seed_{args.seed}.json'), 'w') as f:
                json.dump(total_train_examples, f, indent=4)
            with open(os.path.join(args.output_dir, f'eval_examples_seed_{args.seed}.json'), 'w') as f:
                json.dump(total_eval_examples, f, indent=4)
        ...
        if os.path.isfile(os.path.join(args.output_dir, f'train_examples_seed_{args.seed}.json')) and \
            ...
            dbpedia_datasets = load_dataset(data_cache_dir, cache_dir=data_cache_dir)
        ...
        if os.path.isfile(os.path.join(args.output_dir, f'train_examples_seed_{args.seed}.json')) and \
            ...
            data_files = {
                "train": "/data/huxingjian/In_Context_Learning/hellaswag/hellaswag_train.jsonl",
                "validation": "/data/huxingjian/In_Context_Learning/hellaswag/hellaswag_val.jsonl",
            }
            hellaswag_datasets = load_dataset('json', data_files=data_files, cache_dir=data_cache_dir)
        ...
        if os.path.isfile(os.path.join(args.output_dir, f'train_examples_seed_{args.seed}.json')) and \
            ...
            xsum_dataset = load_dataset(data_cache_dir, cache_dir=data_cache_dir)
...
def get_test(args, n= 500, seed = 0, template_dir=""):
    
    if args.test_data in ["imdb"]:
        label_map = {0: "negative", 1: "positive"}
        label_map_temp = {"negative": 0, "positive": 1}
    elif args.test_data in ['contrast_boolq']:
        label_map = {0: "no", 1: "yes"}
        label_map_temp = {"no": 0 , "yes": 1}
    
    dataset = args.test_data
    data_path = os.path.join("data", template_dir,
                             "{}_{}_{}_{}.jsonl".format(args.test_data, n, seed, "test"))
    data = []
    with open(data_path, "r") as f:
        for line in f:
            dp = json.loads(line)
            data.append(dp)
    processed_eval_examples = process_test_examples(data,label_map_temp)
    
    all_eval_text_to_encode = []
    for item in data:
        all_eval_text_to_encode.append(item["input"])    


    return processed_eval_examples, all_eval_text_to_encode, label_map

def get_train(args, n= 500, seed = 0, template_dir=""):
    ...
    data_path = os.path.join("data", template_dir,
                            "{}_{}_{}_{}.jsonl".format(args.task_name, n, seed, "train"))
    data = []
    with open(data_path, "r") as f:
        for line in f:
            dp = json.loads(line)
            data.append(dp)
    processed_train_examples = process_test_examples(data,label_map_temp)
    
    all_train_text_to_encode = []
    for item in data:
        all_train_text_to_encode.append(item["input"])    
    
    def format_example(example,label_map,**kwargs):
        return example['input'], f"{label_map[example['label']]}"

    return processed_train_examples, all_train_text_to_encode, format_example, label_map