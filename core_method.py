...
def evaluate_node_influence(args):
    ...
    """
    graph, node, selected_node, rand_iter = args
    if node in selected_node:
        return node, 0
    else:
...
def infmax(embeddings, select_num, k = 10, rand_iter=10): 
    ...
    with Pool(processes=16) as pool:
        for i in range(select_num-1):
            max_influence = 0 
            best_node = None
            nodes_to_evaluate = [(graph, node, selected_node.copy(), rand_iter) for node in graph.nodes() if node not in selected_node]
            results = pool.map(evaluate_node_influence, nodes_to_evaluate)
            for node, influence in results:
